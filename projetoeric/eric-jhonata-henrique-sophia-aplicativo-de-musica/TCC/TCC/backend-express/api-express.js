...
app.post('/cadastrarcompra', async (req, res) => {
  ...
  let novaCompra = new Compras(id_usuario);

  try {
    const connection = await usarConexao();
    const [rows] = await connection.query('call mydatabase.cadastro_compra(?);',
      [id_usuario]
    );
    connection.release();
    res.status(201).json({ message: 'Compra cadastrada com sucesso!', novaCompra: novaCompra });
  } catch (error) {
    console.error('Erro ao inserir compra:', error);
    res.status(500).json({ error: 'Erro ao inserir a compra no banco de dados' });
  }
});
